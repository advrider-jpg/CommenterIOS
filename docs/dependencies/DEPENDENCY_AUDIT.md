...
- `Package.swift` declares package products for every third-party source import
  ...
  `CodableCSV`, `CoreXLSX`, `GRDB`, and `ZIPFoundation`.
...
- `CommenterImportExport` imports `CodableCSV`, `CoreXLSX`, and
...
| Legacy XLS import | `SpreadsheetImportFile.swift` uses a fixture-limited local OLE compound-file extractor and narrow BIFF subset decoder after CI exposed a process-level crash path through OLEKit. | Recorded exception. Full `.xls` parity still needs `libxls`/equivalent evaluation; unsupported compound files must surface as unreadable rather than falling through to crash-prone dependency behavior. |
...
Remaining risk:
Legacy `.xls` parity is not complete merely because narrow OLE/BIFF fixture
support exists. Full production BIFF cell decoding/writing still needs either mature
